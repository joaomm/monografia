...
\subsubsection{Pedido de Tarefa}
...
através de um novo método. Podemos dizer que o método fez um ``Pedido de Tarefa''.

O exemplo abaixo ilustra métodos de uma classe \textit{CalculaReceita}, cujo código poderia se beneficiar com um Pedido de Tarefa.
...
importantes: (i) o método \textit{calculaTotalDeSalarios} ficou muito simples só fazendo um Pedido de Tarefa para a classe \textit{Empregado}; 
...
\subsubsection{Objeto Moderador}
...
Associada com o objetivo de minimização de dependências e a criação de classes com única responsabilidade, uma maneira de contornar 
esse cenário é criar uma classe que possa fazer essa relação entre classes. De maneira bastante análoga ao desenvolvimento do Objeto Método 
(\ref{objeto_metodo}), o intuito é transferir a atividade complexa de moderar a comunicação entre objetos diferentes para um novo ``objeto 
moderador''.
